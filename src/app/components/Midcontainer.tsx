...
function Midcontainer() {
    ...
    useEffect(() => {
        let getCategory = async () => {
            let response1 = await axios.get(`${BaseUrl}/news?populate=*&filters[ispopolarnews][$eq]=true&sort[0]=id:desc`);
            let response = await axios.get(`${BaseUrl}/news?populate=*&filters[isfreshnews][$eq]=true&sort[0]=id:desc&pagination[limit]=1`);
                        ...
                        <div className="basis-[75%]  items-center justify-center">
                            {freshnews && freshnews.map((item: any) =>
                                ...
                                <div key={item.id} className="flex flex-col ">
                                    ...
                                    <img src={imageUrl(item?.attributes?.coverimage?.data?.attributes?.url)} alt="Your Image" className="w-full h-auto" />
                                    <h3 className="ml-2 font-normal text-xl">{item?.attributes?.title}</h3>
                        ...
                        <div className="basis-[25%]">
                            ...
                            {polularnews && polularnews.map((item: any) =>
                                ...
                                <div key={item.id} className="flex gap-1 items-center">
                                    {/* <span className="ml-2 text-[28px]">{item?.attributes?.number}</span> */}
                                    <img src={imageUrl(item?.attributes?.coverimage?.data?.attributes?.url)} alt="Your Image" className="w-[60px] h-[60px] rounded-lg" />
